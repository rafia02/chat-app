...
export default function LeftRail() {
  return (
    <div className="flex w-24 flex-col items-center justify-between border-r border-[#1B2233] bg-[#0A0F1C] py-8">
      <div className="space-y-5">
        ...
        {menus.map((Icon, index) => (
          <button
            key={index}
            className="flex h-12 w-12 items-center justify-center rounded-xl text-slate-400 transition hover:bg-[#151D31] hover:text-white"
          >
            <Icon size={22} />
          </button>
        ))}
